...
"""DNS Analyzer - Sniffer/Analyzer

Description: Detects potential DNS data-embedding traffic (live capture or offline PCAP analysis).
...
# Detect base64/base64url-ish strings (heuristic)
BASE_REGEX = r'^[A-Za-z0-9+/_\-=]{8,}$'
...
class Sniffer:
    ...
    def analyze_pcap(self, pcap_path: str):
        """Analyze DNS traffic from a .pcap file (offline mode, no raw sockets)."""
        try:
            import dpkt  # type: ignore
        except Exception as exc:
            raise RuntimeError(
                "Offline PCAP analysis requires 'dpkt'. Install it with: pip install dpkt"
            ) from exc

        def iter_dns_payloads():
            try:
                f = open(pcap_path, 'rb')
            except OSError as exc:
                raise FileNotFoundError(f"Cannot open PCAP file: {pcap_path}") from exc
            ...
            with f:
                ...
                for _ts, buf in pcap:
                    try:
                        ...
                        # UDP DNS
                        if isinstance(l4, dpkt.udp.UDP) and (l4.sport == DNS_PORT or l4.dport == DNS_PORT):
                            if l4.data:
                                yield bytes(l4.data)
                            continue

                        # TCP DNS (best-effort: assumes complete message in one segment)
                        if isinstance(l4, dpkt.tcp.TCP) and (l4.sport == DNS_PORT or l4.dport == DNS_PORT):
                            if len(l4.data) >= 2:
                                dns_len = struct.unpack('!H', l4.data[:2])[0]
                                dns_payload = l4.data[2:2 + dns_len]
                                if dns_payload:
                                    yield bytes(dns_payload)
                            continue
                    except (dpkt.dpkt.UnpackError, ValueError, OSError):
                        continue

        analyzed = 0
        flagged = 0
        for dns_payload in iter_dns_payloads():
            ...
            if probability > self.limit_prob:
                flagged += 1
                print_with_time(f"[PCAP] {spy_domain} | p={probability:.2f}")

        print_with_time(f"[Info] PCAP analyzed: {analyzed} DNS messages, flagged: {flagged}")
    ...
    def sniff_dns(self):
        ...
        if dns_payload:
            ...
            probability, spy_domain = self.analyze_dns(dns_payload)
            if (probability > self.limit_prob):
                # Maybe, this sniffed message belongs to DNS data-embedding traffic
                self.packets[-1] += 1
    ...
    def analyze_dns(self, raw):
        """ 
            Analyzes DNS payload for the presence of data-embedding patterns.
    ...
    def check_fqdn(self, domain) -> bool:
        """
            Checks FQDN for base64/base64url-like label payload.
        """
        payload = b''.join(domain.label[:-2])
        if not payload:
            return False
        ...
        if not self.base_pattern.fullmatch(text):
            return False

        # Attempt urlsafe base64 decode with padding normalization.
        # If it decodes cleanly, it's a strong signal of embedded/encoded data.
        try:
            import base64
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="DNS Analyzer: sniffer script for detecting suspicious traffic")

    mode = parser.add_mutually_exclusive_group(required=True)
    mode.add_argument('-g', '--gateway', dest='ip', type=str,
                      help='Specifies the gateway address (live capture via raw socket)')
    mode.add_argument('-i', '--pcap-in', dest='pcap_in', type=str,
                      help='Analyze an existing .pcap file (offline mode)')

    parser.add_argument('-p', '--limit-prob', dest='prob', type=float, default=0.5,
                        help='Specifies the limit probability for suspicious traffic detection')
    ...
    if args.ip and not (re.compile(IP_REGEX).match(args.ip)):
    ...
    s = Sniffer(filename=args.path, probability=args.prob)

    if args.pcap_in:
        print_with_time(f"[Info] Offline PCAP analysis: {args.pcap_in}")
        try:
            s.analyze_pcap(args.pcap_in)
        except (FileNotFoundError, RuntimeError) as exc:
            print_with_time(f"[Error] {exc}")
            sys.exit(2)
    else:
